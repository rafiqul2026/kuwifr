...
/**
 * Deterministic URL Side Extractor:
 * Guaranteed to run on every render pass.
 * - If pos=R / side=right -> returns 'RIGHT'
 * - If pos=L / side=left -> returns 'LEFT'
 */
const getSideFromLocation = (search) => {
  try {
    const raw = search || (typeof window !== 'undefined' ? window.location.search : '');
    const params = new URLSearchParams(raw);
    const val = (params.get('pos') || params.get('side') || params.get('position') || '').trim().toUpperCase();

    if (val === 'R' || val === 'RIGHT' || val === '2') {
      return 'RIGHT';
    }
    if (val === 'L' || val === 'LEFT' || val === '1') {
      return 'LEFT';
    }
  } catch (e) {
    console.error('Error reading position param:', e);
  }
  return 'LEFT';
};

/**
 * Deterministic URL Sponsor Extractor
 */
const getSponsorFromLocation = (search) => {
  try {
    const raw = search || (typeof window !== 'undefined' ? window.location.search : '');
    const params = new URLSearchParams(raw);
...
const RegisterPage = () => {
  ...
  // Read directly from URL on every render so link changes immediately reflect in the banner
  const activeSide = useMemo(() => getSideFromLocation(location.search), [location.search]);
  const activeSponsor = useMemo(() => getSponsorFromLocation(location.search), [location.search]);
    ...
    <div className={styles.authPage}>
      <div className={styles.authContainer}>
        <div className={styles.authCard}>
          ...
          {/* Dynamic Sponsor Banner: ONLY Sponsor ID & Side (No Sponsor Name) */}
          {activeSponsor && (
            <div
            ...
            >
              ...
              <div>
                ...
                >
                  Sponsor ID: {activeSponsor} | Side: {activeSide}
                </strong>
              </div>
            </div>
          )}
