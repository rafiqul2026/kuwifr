...
class BinaryService {
  ...
  /**
   * Authoritative, READ-ONLY projection of a node's true matched/available
   * volume under the current first-pair-2:1 + 1:1-thereafter rule, computed
   * directly from leftVolume/rightVolume — independent of whether
   * calculateMatching() has actually been re-triggered enough times to catch
   * up to it yet. calculateMatching only re-checks for more pairs on the
   * NEXT volume-adding event for this node, so after a lopsided first pair
   * (e.g. Left far ahead of Right), the node's OWN stored pairCount /
   * matchingVolume / availableLeftVolume / availableRightVolume can sit
   * "behind" the true total for a long time if no further order happens to
   * touch this node — real case: Sofiya (KFR782349), Left 10,000 / Right
   * 7,500, only 1 of the true 7 possible pairs had actually been triggered,
   * so her stored matchingVolume read 1,000 instead of 7,000 and her stored
   * available volumes read 8,000/6,500 instead of the true 2,000/500.
   *
   * This uses the EXACT same target formula reconcileUnderpaidMatchingIncome
   * above uses to decide what income is actually owed — but only computes
   * it, never writes to the node or credits anything — so DISPLAY-only
   * consumers (dashboard cards) can always show the true current state
   * without needing a reconciliation run or another order to arrive first.
   * The actual income-paying engine (calculateMatching /
   * reconcileUnderpaidMatchingIncome) is untouched by this — real income
   * still only pays out when one of those actually runs, by design.
   */
  async getAuthoritativeMatchState(node) {
    const SettingsService = require('./settings.service');
    const matchingCfg = await SettingsService.getMatching();
    const UNIT = matchingCfg.unitValue;
    const SMALL = matchingCfg.firstPairSmallUnits;
    ...
    // Not yet eligible for even the first pair — nothing consumed, full
    // leftVolume/rightVolume still sits unmatched.
    if (!eligible) {
      return { matchedKbp: 0, targetUnits: 0, availableLeft: L, availableRight: R };
    }
    ...
    return {
      matchedKbp: targetUnits * UNIT,
      targetUnits,
      availableLeft: heavyIsLeft ? correctedHeavyAvail : correctedLightAvail,
      availableRight: heavyIsLeft ? correctedLightAvail : correctedHeavyAvail
    };
  }
