...
const { getAmountPayable } = require('../constants/insurancePlans');
...
exports.completePackagePurchase = async (req, res) => {
  try {
    ...
    if (mongoose.Types.ObjectId.isValid(packageId)) {
      chosenPkg = await Package.findById(packageId).select('name type price kbp dailyCap includedProductIds').lean();
    ...
    // Amount payable is computed here, never taken from the client: the
    // package price, or the insurance installment if the member picked this
    // package's insurance plan as their product.
    const payable = chosenPkg ? getAmountPayable(chosenPkg, productsArray) : null;
    if (!chosenPkg && productsArray.some((p) => String(p?.productId || '').startsWith('insurance-'))) {
      return res.status(400).json({ success: false, message: 'Please refresh the page and select your package again.' });
    }
    ...
    const newPurchase = await PackagePurchase.create({
      ...
      packageName: chosenPkg?.name || packageName,
      packagePrice: payable ? payable.amount : Number(packagePrice),
      targetPackagePrice: chosenPkg ? chosenPkg.price : Number(packagePrice),
      isInsurance: payable ? payable.isInsurance : false,
      kbpPoints: chosenPkg ? chosenPkg.kbp : (Number(kbpPoints) || 0),
      dailyBinaryCap: chosenPkg ? chosenPkg.dailyCap : (Number(dailyBinaryCap) || 0),
...
exports.completePackageUpgrade = async (req, res) => {
  try {
    ...
    // or its insurance installment if the member picked the insurance plan.
    // KBP is always 0 for an upgrade — no income is generated by it, only
    // the capping ceiling and bundled product(s) change.
    const payable = getAmountPayable(targetPkg, productsArray);
    const amountPayable = payable.amount;
    ...
    const newPurchase = await PackagePurchase.create({
      ...
      isInsurance: payable.isInsurance,
      kbpPoints: 0,
...
exports.approvePackagePurchase = async (req, res) => {
  ...
  try {
    ...
    // What the member actually paid: the package price, or the insurance
    // installment (fixed server-side at submission) when they chose the
    // insurance plan. The member's tier (user.packagePrice) is always the
    // package price — only revenue records use the amount paid.
    const amountPaid = purchase.isInsurance ? purchase.packagePrice : authoritativePrice;
    ...
    const newOrder = await Order.create([{
      ...
      products: [{
        name: purchase.isInsurance
          ? `${resolvedPackage.name} + ${purchase.selectedProducts?.[0]?.name || 'Insurance'}`
    ...
    try {
      await Notification.create({
        ...
        message: `Your payment of ₹${amountPaid} for ${resolvedPackage.name} has been verified. Your account is now ACTIVE.`,
        icon: '✅',
        color: '#16a34a',
        action: '/member/dashboard',
        actionLabel: 'Go to Dashboard'
