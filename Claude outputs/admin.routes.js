...
// repurchaseBalance and inflated totalIncome ("Total Lifetime Income") by
// the transferred amount. Deploying the code fix does NOT retroactively
// correct already-saved wallet balances — this is that one-time
// correction. Pass ?dryRun=true to preview exactly what would change with
// zero writes. Idempotent — a repaired transaction no longer matches the
// detection query, so a second run finds nothing left to fix. See
// WalletService.repairSystemTransferRouting for full detection/repair
// logic and the manual-review case (member already spent the erroneous
// balance).
router.post('/wallet/repair-transfer-routing', async (req, res, next) => {
  try {
    const WalletService = require('../services/wallet.service');
    const dryRun = req.query.dryRun === 'true' || req.body?.dryRun === true;
    const result = await WalletService.repairSystemTransferRouting({ dryRun });
    res.json({
      success: true,
      message: dryRun
        ? `DRY RUN: ${result.totalWouldRepair} of ${result.totalFound} affected transfer(s) would be repaired (${result.totalNeedsReview} need manual review). No changes were made.`
        : `${result.totalRepaired} of ${result.totalFound} affected transfer(s) repaired (${result.totalNeedsReview} need manual review).`,
      data: result
    });
  } catch (error) {
    next(error);
