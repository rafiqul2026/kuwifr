...
class WalletService {
  ...
  /**
   * One-time data repair for "Transfer to Repurchase Wallet" transactions
   * made BEFORE the transferToRepurchase() routing fix above. Before that
   * fix, every such transfer's credit leg was mis-routed into incomeBalance
   * instead of repurchaseBalance (net-zero visible change on the Income
   * Wallet, Repurchase Wallet never actually credited) and unconditionally
   * inflated wallet.totalIncome ("Total Lifetime Income" on the Wallet
   * page) by the transferred amount, even though a transfer between a
   * member's own wallets isn't new income.
   *
   * Detection: every already-broken transfer credit is a CREDIT,
   * source:'SYSTEM', walletType:'INCOME' WalletTransaction with amount > 0
   * (the only other 'SYSTEM' CREDIT — WalletService.verifyWallet's audit
   * entry — is always amount: 0, so it never matches). Deploying the code
   * fix does NOT retroactively correct these — Mongoose/application code
   * changes never rewrite already-saved documents.
   *
   * Correction per affected transaction: incomeBalance -= amount,
   * repurchaseBalance += amount, totalIncome -= amount (floored at 0), then
   * the transaction's own walletType is updated to 'REPURCHASE' and its
   * description corrected — which also makes this naturally idempotent,
   * since a repaired transaction no longer matches the detection query on
   * a re-run. Only ever touches incomeBalance/repurchaseBalance/
   * totalIncome — never totalWithdrawn/totalRepurchased or any
   * IncomeTransaction/Order record, so nothing already paid out or spent
   * elsewhere is clawed back.
   *
   * Pass dryRun:true (the default) to preview with zero writes.
   */
  async repairSystemTransferRouting({ dryRun = true } = {}) {
    const affected = await WalletTransaction.find({
      type: 'CREDIT',
      source: 'SYSTEM',
      walletType: 'INCOME',
      amount: { $gt: 0 }
      ...
      .sort({ createdAt: 1 })
      .populate('userId', 'fullName sponsorId email');

    const results = [];

    for (const txn of affected) {
      const wallet = await Wallet.findById(txn.walletId);
      const member = txn.userId && txn.userId._id ? txn.userId : null;
      const entry = {
        transactionId: txn.transactionId,
        userId: member ? member._id : txn.userId,
        memberName: member ? member.fullName : undefined,
        memberSponsorId: member ? member.sponsorId : undefined,
        amount: txn.amount,
        createdAt: txn.createdAt
      };

      if (!wallet) {
        results.push({ ...entry, status: 'SKIPPED_NO_WALLET' });
        continue;
      }

      // If the member has since withdrawn or spent the erroneously-inflated
      // Income Wallet balance, subtracting the correction now would push
      // incomeBalance negative. Don't force that — flag it for manual
      // review instead of silently clamping, since that would understate
      // how much this member actually needs reconciled.
      if (wallet.incomeBalance - txn.amount < 0) {
        results.push({
          ...entry,
          status: 'NEEDS_MANUAL_REVIEW',
          currentIncomeBalance: wallet.incomeBalance,
          reason: 'Correcting this would take the Income Wallet negative — the erroneous extra balance appears to have already been withdrawn or spent.'
        });
        continue;
      }

      if (!dryRun) {
        wallet.incomeBalance -= txn.amount;
        wallet.repurchaseBalance += txn.amount;
        wallet.totalIncome = Math.max(0, (wallet.totalIncome || 0) - txn.amount);
        await wallet.save();

        txn.walletType = 'REPURCHASE';
        txn.description = 'Transfer from Income Wallet';
        await txn.save();
      }

      results.push({ ...entry, status: dryRun ? 'WOULD_REPAIR' : 'REPAIRED' });
    }

    return {
      dryRun,
      totalFound: affected.length,
      totalRepaired: results.filter((r) => r.status === 'REPAIRED').length,
      totalWouldRepair: results.filter((r) => r.status === 'WOULD_REPAIR').length,
      totalNeedsReview: results.filter((r) => r.status === 'NEEDS_MANUAL_REVIEW').length,
      results
    };
  }
