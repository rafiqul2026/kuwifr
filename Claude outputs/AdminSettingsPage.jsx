...
const AdminSettingsPage = () => {
  ...
  // Repairs "Transfer to Repurchase Wallet" transactions made before the
  // Sept 2026 routing fix — before that fix, the transfer's credit leg was
  // mis-routed into incomeBalance instead of repurchaseBalance (Repurchase
  // Wallet never actually received the funds) and inflated "Total Lifetime
  // Income" by the transferred amount, even though a transfer between a
  // member's own wallets isn't new income. Deploying the code fix does NOT
  // retroactively correct already-saved balances — this is that one-time
  // correction. Never touches totalWithdrawn/totalRepurchased or any
  // IncomeTransaction/Order record. Idempotent — safe to run more than
  // once; a second run should find nothing left to repair.
  const handleRepairWalletTransferRouting = async (dryRun = false) => {
    setIsRepairingWalletTransfers(true);
    if (!dryRun) setWalletTransferRepairResult(null);
    try {
      const res = await api.post(`/api/admin/wallet/repair-transfer-routing${dryRun ? '?dryRun=true' : ''}`);
      const summary = res.data?.data;
      setWalletTransferRepairResult(summary);
      showNotification(res.data?.message || 'Wallet transfer repair complete.', 'success');
    } catch (err) {
      showNotification(err.response?.data?.message || 'Wallet transfer repair failed.', 'error');
    } finally {
      setIsRepairingWalletTransfers(false);
    }
  };
              ...
              <div className={styles.sectionBlock}>
                ...
                <div className={styles.sectionBlock} style={{ marginTop: '24px' }}>
                  ...
                  <div className={styles.formGrid}>
                    ...
                    <div className={styles.maintenanceCard}>
                      <span className={styles.maintenanceLabel}>Repair Wallet Transfer Routing (Transfer to Repurchase Wallet)</span>
                      <p className={styles.maintenanceHelp}>
                        Fixes "Transfer to Repurchase Wallet" transactions made before the routing fix: the transfer's
                        credit leg was landing in the Income Wallet instead of the Repurchase Wallet (so the
                        Repurchase Wallet never actually received the funds) and was inflating "Total Lifetime
                        Income" by the transferred amount, even though moving money between a member's own wallets
                        isn't new income. This corrects the Income Wallet, Repurchase Wallet, and Total Lifetime
                        Income for every affected member. It never touches totalWithdrawn/totalRepurchased or any
                        IncomeTransaction/Order record — nothing already paid out or spent elsewhere is clawed back.
                        A member who has since withdrawn or spent the erroneous extra Income Wallet balance is
                        flagged as "needs manual review" instead of being forced negative. Safe to run more than
                        once — a repaired transfer won't be repaired again.
                      </p>
                      <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginTop: '10px' }}>
                        <button
                          type="button"
                          className={styles.testEmailBtn}
                          onClick={() => handleRepairWalletTransferRouting(true)}
                          disabled={isRepairingWalletTransfers}
                        >
                          {isRepairingWalletTransfers ? 'Working...' : '👁️ Preview (Dry Run)'}
                        </button>
                        <button
                          type="button"
                          className={styles.testEmailBtn}
                          onClick={() => handleRepairWalletTransferRouting(false)}
                          disabled={isRepairingWalletTransfers}
                        >
                          {isRepairingWalletTransfers ? 'Repairing...' : '🛠️ Repair Wallet Transfers'}
                        </button>
                      </div>
                      {walletTransferRepairResult && (
                        <div className={styles.maintenanceHelp} style={{ marginTop: '8px' }}>
                          <p style={{ margin: 0 }}>
                            {walletTransferRepairResult.dryRun ? 'DRY RUN — ' : ''}
                            {walletTransferRepairResult.dryRun
                              ? `${walletTransferRepairResult.totalWouldRepair} of ${walletTransferRepairResult.totalFound} would be repaired`
                              : `${walletTransferRepairResult.totalRepaired} of ${walletTransferRepairResult.totalFound} repaired`}
                            {' '}· {walletTransferRepairResult.totalNeedsReview} need manual review
                          </p>
                          {walletTransferRepairResult.results?.length > 0 && (
                            <ul style={{ margin: '8px 0 0', paddingLeft: '18px', maxHeight: '220px', overflowY: 'auto' }}>
                              {walletTransferRepairResult.results.map((r) => (
                                <li key={r.transactionId} style={{ marginBottom: '6px' }}>
                                  <strong>{r.memberName || r.memberSponsorId || r.userId}</strong>
                                  {r.memberSponsorId ? ` (${r.memberSponsorId})` : ''} — ₹{r.amount} on{' '}
                                  {r.createdAt ? new Date(r.createdAt).toLocaleString() : ''} —{' '}
                                  <strong style={{ color: r.status === 'NEEDS_MANUAL_REVIEW' ? '#b3261e' : undefined }}>
                                    {r.status}
                                  </strong>
                                  {r.reason ? ` — ${r.reason}` : ''}
                                </li>
